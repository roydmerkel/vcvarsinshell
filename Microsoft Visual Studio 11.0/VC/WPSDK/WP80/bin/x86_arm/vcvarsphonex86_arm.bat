...
@if not "%WindowsPhoneKitDir%" == "" (
	@set "PATH=%WindowsPhoneKitDir%bin;%WindowsPhoneKitDir%bin\x86;%PATH%"
	@set "INCLUDE=%WindowsPhoneKitDir%include;%WindowsPhoneKitDir%include\abi;%WindowsPhoneKitDir%include\mincore;%WindowsPhoneKitDir%include\minwin;%WindowsPhoneKitDir%include\wrl;%INCLUDE%"
	@set "LIB=%WindowsPhoneKitDir%lib\ARM;%LIB%"
	@set "LIBPATH=%WindowsPhoneKitDir%Windows MetaData;%LIBPATH%"
)

@if not "%WindowsPhoneSdkDir%" == "" (
	@set "PATH=%WindowsPhoneSdkDir%v8.0\Tools\XAP Deployment;%PATH%"
)

@if not "%ExtensionSDKDir%" == "" (
	@set "LIBPATH=%ExtensionSDKDir%\Microsoft.VCLibs\11.0\References\CommonConfiguration\neutral;%LIBPATH%"
)
