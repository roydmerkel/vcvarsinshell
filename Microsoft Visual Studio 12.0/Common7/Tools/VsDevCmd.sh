...
if [ "${VS120COMNTOOLS:-}" == "" ]; then
	...
	if [ "${VSINSTALLDIR}" == "" ]; then 
		...
		if [ "${WindowsSDK_ExecutablePath_x86:-}" != "" ]; then 
			export "PATH=${WindowsSDK_ExecutablePath_x86}${PATH:+:${PATH}}"
		fi
		...
		if [ "${WindowsSdkDir:-}" != "" ]; then
			...
			export LIB="${WindowsSdkDir}lib/winv6.3/um/x86${LIB:+:${LIB}}"
		...
		# Add path to TypeScript Compiler
		if [ -e "${ProgramFiles}/Microsoft SDKs/TypeScript" ]; then 
			export PATH="${ProgramFiles}/Microsoft SDKs/TypeScript${PATH:+:${PATH}}"
		fi
		if [ -e "${ProgramFilesx86}/Microsoft SDKs/TypeScript" ]; then 
			export PATH="${ProgramFilesx86}/Microsoft SDKs/TypeScript${PATH:+:${PATH}}"
		fi

		if [ -e "${VSINSTALLDIR}VSTSDB/Deploy" ]; then
			export PATH="${VSINSTALLDIR}VSTSDB/Deploy${PATH:+:${PATH}}"
		fi
		...
		if [ -e "${VCINSTALLDIR}ATLMFC/LIB" ]; then
			export LIB="${VCINSTALLDIR}ATLMFC/LIB${LIB:+:${LIB}}"
		fi
		if [ -e "${VCINSTALLDIR}LIB" ]; then 
			export LIB="${VCINSTALLDIR}LIB${LIB:+:${LIB}}"
		fi

		# LIBPATH
		# -------
		if [ -e "${VCINSTALLDIR}ATLMFC/LIB" ]; then 
			export LIBPATH="${VCINSTALLDIR}ATLMFC/LIB${LIBPATH:+:${LIBPATH}}"
		fi
		if [ -e "${VCINSTALLDIR}LIB" ]; then 
			export LIBPATH="${VCINSTALLDIR}LIB${LIBPATH:+:${LIBPATH}}"
		fi
		if [ -e "${FrameworkDir}${Framework40Version}" ]; then 
