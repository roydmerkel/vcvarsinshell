...
GetWindowsSdkDirHelper32() {
	local regqueryRes=$("${reg}" query "${1}\\SOFTWARE\\Microsoft\\Microsoft SDKs\\Windows\\v8.0" /v "InstallationFolder" 2>/dev/null | tr -d '\r')
...
GetWindowsSdkDirHelper64() {
	local regqueryRes=$("${reg}" query "${1}\\SOFTWARE\Wow6432Node\Microsoft\Microsoft SDKs\Windows\v8.0" /v "InstallationFolder" 2>/dev/null | tr -d '\r')
	if [[ -n "$regqueryRes" ]]; then
		while read -r i j k; do
			if [ "${i}" == "InstallationFolder" ]; then
				export WindowsSdkDir="$(winpath -u "${k}")"
...
GetWindowsSdkDirHelper32_old() {
	local regqueryRes=$("${reg}" query "${1}\\SOFTWARE\\Microsoft\\Microsoft SDKs\\Windows\\v8.0a" /v "InstallationFolder" 2>/dev/null | tr -d '\r')
	if [[ -n "$regqueryRes" ]]; then
		while read -r i j k; do
			if [ "${i}" == "InstallationFolder" ]; then
				export WindowsSdkDir_old="$(winpath -u "${k}")"
			fi
		done <<< "$regqueryRes"
	fi
...
GetWindowsSdkDirHelper64_old() {
	local regqueryRes=$("${reg}" query "${1}\\SOFTWARE\\Wow6432Node\\Microsoft\\Microsoft SDKs\\Windows\\v8.0a" /v "InstallationFolder" 2>/dev/null | tr -d '\r')
	if [[ -n "$regqueryRes" ]]; then
		while read -r i j k; do
			if [ "${i}" == "InstallationFolder" ]; then
				export WindowsSdkDir_old="$(winpath -u "${k}")"
...
GetWindowsSdkDir_old() {
	export WindowsSdkDir_old=
	if ! GetWindowsSdkDirHelper32_old HKLM; then 
		if ! GetWindowsSdkDirHelper32_old HKCU; then
			if ! GetWindowsSdkDirHelper64_old HKLM; then 
				GetWindowsSdkDirHelper64_old HKCU
			fi
		fi
	fi
	return 0
...
GetWindowsSdkDirHelper32_35() {
	local regqueryRes=$("${reg}" query "${1}\\SOFTWARE\\Microsoft\\Microsoft SDKs\\Windows\\v8.0A\\WinSDK-NetFx35Tools" /v "InstallationFolder" 2>/dev/null | tr -d '\r')
	if [[ -n "$regqueryRes" ]]; then
		while read -r i j k; do
			if [ "${i}" == "InstallationFolder" ]; then
				export WindowsSdkDir_35="$(winpath -u "${k}")"
			fi
		done <<< "$regqueryRes"
	fi
	if [ "${WindowsSdkDir_35:-}" == "" ]; then 
		return 1
	fi
	return 0
}
...
GetWindowsSdkDirHelper64_35() {
	local regqueryRes=$("${reg}" query "${1}\\SOFTWARE\\Wow6432Node\\Microsoft SDKs\\Windows\\v8.0A\\WinSDK-NetFx35Tools" /v "InstallationFolder" 2>/dev/null | tr -d '\r')
	if [[ -n "$regqueryRes" ]]; then
		while read -r i j k; do
			if [ "${i}" == "InstallationFolder" ]; then
				export WindowsSdkDir_35="$(winpath -u "${k}")"
...
GetWindowsSdkDir_35() {
	export WindowsSdkDir_35=
	if ! GetWindowsSdkDirHelper32_35 HKLM; then
		if ! GetWindowsSdkDirHelper32_35 HKCU; then 
			if ! GetWindowsSdkDirHelper64_35 HKLM; then 
				GetWindowsSdkDirHelper64_35 HKCU
			fi
		fi
	fi
	return 0
...
GetExtensionSdkDir() {
	...
	if [ -e "${ProgramFiles}/Microsoft SDKs/Windows/v8.0/ExtensionSDKs/Microsoft.VCLibs/11.0/SDKManifest.xml" ]; then 
		export ExtensionSdkDir="${ProgramFiles}/Microsoft SDKs/Windows/v8.0/ExtensionSDKs"
	fi
	if [ -e "${ProgramFilesx86}/Microsoft SDKs/Windows/v8.0/ExtensionSDKs/Microsoft.VCLibs/11.0/SDKManifest.xml" ]; then
...
GetFSharpInstallDirHelper32() {
	local regqueryRes=$("${reg}" query "${1}\\SOFTWARE\\Microsoft\\VisualStudio\\11.0\\Setup\\F#" /v "ProductDir" 2>/dev/null | tr -d '\r')
	if [[ -n "$regqueryRes" ]]; then
		while read -r i j k; do
...
GetFSharpInstallDirHelper64() {
	local regqueryRes=$("${reg}" query "${1}\\SOFTWARE\\Wow6432Node\\Microsoft\\VisualStudio\\11.0\\Setup\\F#" /v "ProductDir" 2>/dev/null | tr -d '\r')
	if [[ -n "$regqueryRes" ]]; then
		while read -r i j k; do
			if [ "${i}" == "ProductDir" ]; then
				export FSHARPINSTALLDIR="$(winpath -u "${k}")"
