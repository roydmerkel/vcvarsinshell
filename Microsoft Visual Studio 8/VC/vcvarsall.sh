...
if [ "${VS80COMNTOOLS:-}" == "" ]; then
    export VS80COMNTOOLS="$(realpath "$(dirname "${dollar0}")/../Common7/Tools")/"
fi
...
amd64() {
    if [ ! -e "$(dirname "${dollar0}")/bin/amd64/vcvarsamd64.sh" ]; then 
        missing
    else
        . "$(dirname "${dollar0}")/bin/amd64/vcvarsamd64.sh"
    fi
}

ia64() {
    if [ ! -e "$(dirname "${dollar0}")/bin/ia64/vcvarsia64.sh" ]; then 
        missing
    else
        . "$(dirname "${dollar0}")/bin/ia64/vcvarsia64.sh"
    fi
}
