...
if [ "${VS110COMNTOOLS}" == "" ]; then 
	...
	if [ "${VSINSTALLDIR}" == "" ]; then
		...
		if [ "${WindowsSdkDir_old:-}" != "" ]; then
			if [ "${PATH:-}" != "" ]; then
				export PATH="${WindowsSdkDir_old}bin/NETFX 4.0 Tools:${WindowsSdkDir_35}:${PATH}"
			else
				export PATH="${WindowsSdkDir_old}bin/NETFX 4.0 Tools:${WindowsSdkDir_35}"
			fi
		fi
		...
		if [ "${PATH:-}" != "" ]; then
			export PATH="${VCINSTALLDIR}BIN/x86_amd64:${PATH}"
		else
			export PATH="${VCINSTALLDIR}BIN/x86_amd64"
		fi
		if [ "${PATH:-}" != "" ]; then
