...
if [ "${VS110COMNTOOLS:-}" == "" ]; then
	...
	. "${VS110COMNTOOLS}VCVarsQueryRegistry.sh" No32bit 64bit
	...
	if [ "${VSINSTALLDIR:-}" == "" ]; then
		...
		if [ "${WindowsSdkDir_old:-}" != "" ]; then
			if [ "${PATH:-}" != "" ]; then
				export PATH="${WindowsSdkDir_old}bin/NETFX 4.0 Tools/x64:${WindowsSdkDir_35}x64:${WindowsSdkDir_old}bin/NETFX 4.0 Tools:${WindowsSdkDir_35}:${PATH}"
			else
				export PATH="${WindowsSdkDir_old}bin/NETFX 4.0 Tools/x64:${WindowsSdkDir_35}x64:${WindowsSdkDir_old}bin/NETFX 4.0 Tools:${WindowsSdkDir_35}"
			fi
		fi

		if [ "${WindowsSdkDir:-}" != "" ]; then
			if [ "${PATH:-}" != "" ]; then
				export PATH="${WindowsSdkDir}bin/x64:${WindowsSdkDir}bin/x86:${PATH}"
			else
				export PATH="${WindowsSdkDir}bin/x64:${WindowsSdkDir}bin/x86"
			fi
			if [ "${INCLUDE:-}" != "" ]; then
			...
			if [ "${LIB:-}" != "" ]; then
				export LIB="${WindowsSdkDir}lib/win8/um/x64:${LIB}"
			else
				export LIB="${WindowsSdkDir}lib/win8/um/x64"
			fi
			if [ "${LIBPATH:-}" != "" ]; then
		...
		if [ -e "${VSINSTALLDIR}Common7/IDE/CommonExtensions/Microsoft/TestWindow" ]; then
			if [ "${PATH:-}" != "" ]; then
				export PATH="${VSINSTALLDIR}Common7/IDE/CommonExtensions/Microsoft/TestWindow:${PATH}"
			else
				export PATH="${VSINSTALLDIR}Common7/IDE/CommonExtensions/Microsoft/TestWindow"
			fi
		fi
		...
		if [ -e "${VCINSTALLDIR}ATLMFC/LIB/amd64" ]; then 
			if [ "${LIB:-}" != "" ]; then
				export LIB="${VCINSTALLDIR}ATLMFC/LIB/amd64:${LIB}"
			else
				export LIB="${VCINSTALLDIR}ATLMFC/LIB/amd64"
			fi
		fi
		if [ "${LIB:-}" != "" ]; then
