...
if [ "${VS110COMNTOOLS:-}" == "" ]; then 
	...
	. "$(dirname "${REALPATH}")/../VCVarsPhoneQueryRegistry.sh" 32bit 64bit
	...
	if [ "${VSINSTALLDIR:-}" == "" ]; then
		...
		if [ "${WindowsPhoneKitDir:-}" != "" ]; then
			...
			if [ "${LIB:-}" != "" ]; then
				export LIB="${WindowsPhoneKitDir}lib/ARM:${LIB}"
			else
				export LIB="${WindowsPhoneKitDir}lib/ARM"
			fi
			if [ "${LIBPATH:-}" != "" ]; then
		...
		if [ "${PATH:-}" != "" ]; then
			export PATH="${VCPhoneToolsRoot}BIN/x86_ARM:${PATH}"
		else
			export PATH="${VCPhoneToolsRoot}BIN/x86_ARM"
		fi
		if [ "${PATH:-}" != "" ]; then
		...
		# ---
		if [ "${LIB:-}" != "" ]; then
			export LIB="${VCPhoneToolsRoot}LIB/ARM:${LIB}"
		else
			export LIB="${VCPhoneToolsRoot}LIB/ARM"
		fi

		# LIBPATH
		# -------
		if [ "${LIBPATH:-}" != "" ]; then
			export LIBPATH="${VCPhoneToolsRoot}LIB/ARM:${LIBPATH}"
		else
			export LIBPATH="${VCPhoneToolsRoot}LIB/ARM"
		fi
		if [ "${LIBPATH:-}" != "" ]; then
