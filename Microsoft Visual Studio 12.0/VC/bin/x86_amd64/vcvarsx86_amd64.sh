...
if [ "${VS120COMNTOOLS:-}" == "" ]; then
	...
	. "${VS120COMNTOOLS}VCVarsQueryRegistry.sh" 32bit 64bit
	...
	if [ "${VSINSTALLDIR:-}" == "" ]; then
		...
		if [ "${WindowsSdkDir:-}" != "" ]; then
			...
			export LIB="${WindowsSdkDir}lib/winv6.3/um/x64${LIB:+:${LIB}}"
		...
		if [ -e "${VCINSTALLDIR}BIN/x86_amd64" ]; then 
			export PATH="${VCINSTALLDIR}BIN/x86_amd64${PATH:+:${PATH}}"
		fi
		...
		if [ -e "${VCINSTALLDIR}ATLMFC/LIB/amd64" ]; then 
			export LIB="${VCINSTALLDIR}ATLMFC/LIB/amd64${LIB:+:${LIB}}"
		fi
		if [ -e "${VCINSTALLDIR}LIB/amd64" ]; then 
			export LIB="${VCINSTALLDIR}LIB/amd64${LIB:+:${LIB}}"
		fi

		# LIBPATH
		# -------
		if [ -e "${VCINSTALLDIR}ATLMFC/LIB/amd64" ]; then 
			export LIBPATH="${VCINSTALLDIR}ATLMFC/LIB/amd64${LIBPATH:+:${LIBPATH}}"
		fi
		if [ -e "${VCINSTALLDIR}LIB/amd64" ]; then 
			export LIBPATH="${VCINSTALLDIR}LIB/amd64${LIBPATH:+:${LIBPATH}}"
		fi
		if [ -e "${FrameworkDir}${Framework40Version}" ]; then
		...
		if [ -e "${FrameworkDir64}${Framework40Version}" ]; then 
			export LIBPATH="${FrameworkDir64}${Framework40Version}${LIBPATH:+:${LIBPATH}}"
		fi
		if [ -e "${FrameworkDir64}${FrameworkVersion64}" ]; then 
			export LIBPATH="${FrameworkDir64}${FrameworkVersion64}${LIBPATH:+:${LIBPATH}}"
		fi
